...
import itertools
import numpy as np
import mmap # for virtual (in RAM) files, used for the snapshot
import matplotlib.pyplot as plt

class Yoko750(instr.Instr):
    def __init__(self, visa_name, visa_library='', installed_channels=[1,2,3,4,9,10,11,12]):
        super(Yoko750, self).__init__(visa_name, visa_library)
        self.visa_instr.timeout = 1000 # in ms (tested with 100 ms, works with chunk size of 2*10010+9+2)
        self.visa_instr.read_termination = '\n'
        self.visa_instr.write_termination = '\n'
        self.visa_instr.send_end = True
        self.visa_instr.query_delay = 1.e-6   # in s (tested with 1.e-9 (1ns) and it works)
        ...
        self.visa_instr.parity = visa.constants.VI_ASRL_PAR_NONE # parity=none define in pyvisa as VI_ASRL_PAR_NONE which is defined = 0
        self.visa_instr.stop_bits = visa.constants.VI_ASRL_STOP_ONE # 1 bit = value 10, 1.5bit=15, 2bit=20
        ...
        INFO("Installed channels: {}. Use optional parameter installed_channels=[1,2,...] to change.".format(installed_channels))
        self.possible_timebases = [5,10,20,50,100,200,500,1e3,2e3,5e3,10e3,20e3,50e3,100e3,200e3,500e3,1e6,2e6,5e6,10e6] #Unit: Samples/s
        self.possible_record_lengths = [1e3, 2.5e3, 5e3, 10e3, 25e3, 50e3, 100e3, 250e3, 500e3, 1e6, 2.5e6, 5e6, 10e6, 25e6, 50e6, 100e6, 250e6, 500e6, 1e9] # Unit : Total Samples
    ...
    def clock_external(self,yes=None):
        ...
        if yes is None:
            ...
            ERR("Error in timebase_source_is_internal(): argument must be None, True or False.")
            return RETURN_ERROR
        ...
        * if argument is an integer, change the averaging type. See line above for allowed values. Finite averaging is rounded to the lower power of 2.
        ...
        if number is None:
            if self.acq_mode() == "AVER":
                ...
                ERR("Box averaging and Envelope acquisition modes not implemented yet.") # TODO
                return RETURN_ERROR
        elif number == 1:
            self.acq_mode("NORM")
            return RETURN_NO_ERROR
            ...
            if self.trigger_mode() in ["SING", "NSIN", "LOG"]:
                ERR("Cannot use averaging when trigger is set to 'SINGle', 'NSINgle' or 'LOG'. Call trigger_mode() with argument 'AUTO', 'ALEVel' or 'NORMal' first.")
                return RETURN_ERROR
            else:
                self.acq_mode("AVER")
            ...
            if self.trigger_mode() in ["SING", "NSIN", "LOG"]:
                ...
                if rounded_number != number:
                    WARN("Number of averages requested ({0}) rounded to the lower power of 2: {1}".format(number,rounded_number))
                self.write("ACQ:AVER:COUN {0}".format(rounded_number))
                return RETURN_NO_ERROR
        else:
            ERR("Error in averaging(): argument is None for query, 1 for no averaging, 0 for infinity, or 2 to 65536 (rounded to the lower power of 2)")
            return RETURN_ERROR
    ...
    def acq_mode(self,mode=None):
        ...
        * if argument is ``NORM``, ``AVER``, ``BAV`` or ``ENV``, change the averaging type accordingly.
        ...
        if mode is None:
            ...
            ERR("Acquisistion mode must be 'NORMal', 'AVERage', 'BAVerage' or 'ENVelope' (short or long form, case-insensitive).")
            return RETURN_ERROR
    ...
    def volt_per_div(self,numtrace=None,value=None):
        ...
        if numtrace is None:
            ...
            ERR("numtrace argument must be None for current trace or a number in {0}".format(self.hardware_channels))
            return RETURN_ERROR
    ...
    def waveformat(self, formattype):
        if formattype.upper() == FORMAT_ASCII.upper() and self._waveformat != FORMAT_ASCII:
            self.write('WAV:FORM ASC')
            self._waveformat == FORMAT_ASCII
        elif formattype.upper() == FORMAT_BYTE.upper() and self._waveformat != FORMAT_BYTE:
            self.write('WAV:FORM BYTE')
            assert self.query("WAV:BITS?") == '8'
            self.write("WAV:BYTE LSBFIRST")
            self._waveformat == FORMAT_BYTE
        elif formattype.upper() == FORMAT_WORD.upper() and self._waveformat != FORMAT_WORD:
            self.write('WAV:FORM WORD')
            assert self.query("WAV:BITS?") == '16'
            self.write("WAV:BYTE LSBFIRST")
            self._waveformat == FORMAT_WORD
        else:
            ERR(f"Parameter must be '{FORMAT_ASCII}' (ASCII), '{FORMAT_BYTE}' (binary 8 bit) or '{FORMAT_WORD}' (binary 16 bit, LSB)")
    ...
    def trigger_source(self,source=None):
        if source is None:
            ...
            if source in self.hardware_channels:
                ...
                ERR("Trigger source must be a valid hardware channel or 'EXTernal', 'LINE', 'TIME', 'PODA' or PODB' (short or long form, case-insensitive).")
                return RETURN_ERROR
        else:
            ERR("Trigger source must be a valid hardware channel or 'EXTernal', 'LINE', 'TIME', 'PODA' or PODB' (short or long form, case-insensitive).")
            return RETURN_ERROR
    ...
    def trigger_mode(self,mode=None):
        if mode is None:
            ...
            ERR("Trigger mode must be 'AUTO', 'ALEVel', 'NORMal', 'SINGle' or 'NSINgle' (short or long form, case-insensitive).")
            return RETURN_ERROR
    ...
    def trigger_level(self,level=None,slope_down=False):
        ...
        if level is None:
            ...
            if not trig_source.upper().startswith(('EXT','LINE','TIME','LOGICA','LOGICB')):
                self.write("TRIG:POS {0}".format(level))
            else:
                ERR("level cannot be adjusted with trig mode 'EXTernal','LINE','TIME',LOGICA' or 'LOGICB'") 
    ...
    def XY(self,XYmode=None,numdisplay=1,xaxis=11):
        if XYmode is None:
            ...
            ERR("Argument must be None (for query), True or False. Parameters: numdisplay=1,2,3 or 4. xaxis=? where ? is channel number")
            return RETURN_ERROR
        ...
        if traces is None:
            ...
            if not isinstance(traces,list):
                if isinstance(traces,int):
                    ...
                    ERR("Example of possible arguments: None, 1, or [1,2,...] (None, int, or list of int)")
                    out = RETURN_ERROR
            for tr in list(traces):
                if isinstance(tr, int):
                    if tr in self.hardware_channels:
                        ...
                        ERR("Trace {0} is not in the list of harware channels.".format(tr))
                        out = RETURN_ERROR
                else:
                    ERR("Example of possible arguments: None, 1, or [1,2,...] (None, int, or list of int)")
                    out = RETURN_ERROR
    ...
    def traces_on(self,traces=None):
        ...
        out = RETURN_NO_ERROR
        if traces is None:
            self.traces_on([t for t in self.hardware_channels if t not in self.active_traces])
        else:
            if not isinstance(traces,list):
                if isinstance(traces,int):
                    traces = [traces]
    ...
    def current_trace(self,numtrace=None):
        ...
        if numtrace is None:
            ...
            try :
                ...
                WARN("Current trace in Yoko750 was a special channel: '{0}'. Changing it to Ch{1}.".format(buff,tr))
                out = self.trace_current
            ...
            if numtrace in self.hardware_channels:
                ...
                ERR("Trace {0} is not in the list of harware channels.".format(numtrace))
                out = RETURN_ERROR
        else:
            ERR("Trace number must be an integer or None for query. (DSP, MATH, etc. not implemented)")
            out = RETURN_ERROR
        return out
    ...
    def get_ascii(self,tracenum=None):
        ...
        self.traces[trace_to_get-1].x = np.arange(N)/self.traces[trace_to_get-1].srate
        self.traces[trace_to_get-1].probe = self.query_ascii_values("CHAN{0}:PROB?".format(trace_to_get))[0]
    ...
    def get_binary(self,tracenum=None):
        ...
        self.traces[trace_to_get-1].x = np.arange(N)/self.traces[trace_to_get-1].srate
        self.traces[trace_to_get-1].probe = self._to_int(self.query("CHAN{0}:PROB?".format(trace_to_get)))
        self.traces[trace_to_get-1].averaging = self.averaging()
        ...
        dataraw = np.array(self.visa_instr.query_binary_values("WAV:SEND?", header_fmt='ieee', datatype='h', is_big_endian=False, delay=None)) #, expect_termination=False)) # datatype 'h' is for short = 2 bytes
    ...
    def get_binary_old(self,tracenum=None):
        ...
        if 2*N+8 > self.visa_instr.chunk_size:
            INFO("You may want to increase visa_instr.chunk_size to 2*{}+8 to accelerate transfer a little".format(N))

        old_timeout = self.visa_instr.timeout
        if N >= 10e3:
            if N >= 100e3:
                if N >= 1e6:
                    WARN("{0} : Getting {1} points from Yoko DL750. May take a while (typ. 1 min for 1e6 pts).".format(time.ctime(),N))
                    if N >= 10e6:
                        if N >= 100e6:
                            if N >= 250e6:
        ...
        self.traces[trace_to_get-1].N = N
        self.write("WAV:STAR 0") #changement Joël et JD -> Définition du premier point de l'acquisition 17Oct16
        self.write("WAV:END {0}".format(N-1))
        # function query_binary_values() from pyvisa module with parameter header_fmt='ieee' removes the IEEE header #<id><data_length><data>
        dataraw = np.array(self.visa_instr.query_binary_values("WAV:SEND?", header_fmt='ieee', datatype='h', is_big_endian=True, delay=None)) # datatype 'h' is for short = 2 bytes
        data = rang * dataraw * 10 / divis + offs
        self.traces[trace_to_get-1].y = data
    ...
    def get_binary_quick(self, tracenum):
        ...
        dataraw = np.array(self.visa_instr.query_binary_values("WAV:SEND?", header_fmt='ieee', datatype='h', is_big_endian=True, delay=None)) # datatype 'h' is for short = 2 bytes
        divis = 24000.0
        data = self.traces[trace_to_get-1].yrange * dataraw * 10 / divis + self.traces[trace_to_get-1].offset
        self.traces[trace_to_get-1].y = data
    ...
    def bandwidth(self,numtrace=None,bandwidth=None):
        ...
        * if ``bandwidth`` = frequency in Hz : sets the bandwidth. (rounded by Yoko DL750 to the nearest possible value
        ...
        if bw_check != bandwidth:
            WARN("Actual bandwidth of channel {0} is {1} Hz instead of requested {2} (rounded by Yoko DL750 to the nearest possible value).".format(tr,bw_check,bandwidth))
            return bw_check
        else:
            return RETURN_NO_ERROR
        ...
        * if ``invert`` = False, do not invert the waveform on the module
        ...
        if invert is None:
            ...
            if tmp == 1:
                ...
                ERR("Value returned by 'CHAN{0}:INV?' is neither '0' or '1' !!!".format(numtrace))
                return RETURN_ERROR
        elif invert is True:
        ...
        * if ``ac_coupled`` = False, DC coupling mode is selected
        ...
        if ac_coupled is None:
            ...
            if tmp == "AC":
                ...
                ERR("Value returned by 'CHAN{0}:COUP?' is neither 'AC' or 'DC. Other values ('GND', 'ACRMS', 'DCRMS', 'TC') are not supported.".format(numtrace))
                return RETURN_ERROR
    ...
    def save_itx(self,filename,traces=None):
        ...
        if traces is None:
            ...
            for t in traces:
                if not isinstance(t,int) or t not in self.active_traces:
                    all_good = False
            if not all_good:
                ERR("``traces`` argument must be None for current trace, the number of an active trace (in {0}), or a list of active traces numbers.".format(self.active_traces))
                return RETURN_ERROR
            tr = traces
        ...
        DEFAULT_COLOR_LIST_ALL = [(31, 119, 180), (174, 199, 232), (255, 127, 14), (255, 187, 120), (44, 160, 44), (152, 223, 138), (214, 39, 40), (255, 152, 150), (148, 103, 189), (197, 176, 213), (140, 86, 75), (196, 156, 148), (227, 119, 194), (247, 182, 210), (127, 127, 127), (199, 199, 199), (188, 189, 34), (219, 219, 141), (23, 190, 207), (158, 218, 229)]
        DEFAULT_COLOR_LIST = DEFAULT_COLOR_LIST_ALL[0::2]
        DEFAULT_COLOR_LIST_FADED = DEFAULT_COLOR_LIST[1::2]
        DEFAULT_COLOR_LIST_BLIND = [(0, 107, 164), (255, 128, 14), (171, 171, 171), (89, 89, 89), (95, 158, 209), (200, 82, 0), (137, 137, 137), (163, 200, 236), (255, 188, 121), (207, 207, 207)]
        ...
        igor_script += 'NewDataFolder/O root:Data:{series}\n'.format(series=self.traces[tr[0]-1].series)
        # igor_script += 'SetDataFolder root:Data:\n'

        for t in tr:
            header += "\t{series}_{label}".format(series=self.traces[t-1].series,label=self.traces[t-1].label)
            ...
            igor_script += '// {dic}'.format(dic={key:value for key, value in self.traces[t-1].__dict__.items() if not key.startswith('__') and not callable(key) and key is not 'x' and key is not 'y'})

        igor_script += 'String/G root:fldrSav0=GetDataFolder(1)\n'
        igor_script += 'SetDataFolder root:Data:{series}:\n'.format(series=self.traces[tr[0]-1].series)
        igor_script += 'Display /W=(13.2,397.4,408.6,605) ' + ','.join(['{series}_{label}'.format(series=self.traces[t-1].series,label=self.traces[t-1].label) for t in tr]) + ' as "{series}"\n'.format(series=self.traces[tr[0]].series)
        igor_script += 'SetDataFolder root:fldrSav0\n'
        igor_script += 'ModifyGraph ' + ','.join([ '='.join(['rgb({series}_{label})'.format(series=self.traces[tr[i]-1].series,label=self.traces[tr[i]-1].label),'{colour}'.format(colour=tuple([c*2**8 for c in colour_list[i]]))]) for i in range(len(tr))]) + '\n'
        ...
        igor_script += 'Legend/C/N={series}Legend/J/F=0/B=1 "'.format(series=self.traces[tr[0]].series) + '\\r'.join(['\\\\s({series}_{label}) {series}_{label}'.format(series=self.traces[t-1].series,label=self.traces[t-1].label) for t in tr]) + '"'

        header += "\nBEGIN"
        footer = "END\n" + "X ".join(dedent(igor_script).splitlines(True))


        X = np.sum(np.array([ np.array([np.eye(len(tr))[n]]).transpose() * self.traces[tr[n]-1].y / self.traces[tr[n]-1].gain for n in range(len(tr))]),1).transpose()
        np.savetxt(filename,X,fmt="\t% .16e",delimiter='',newline='\n',comments='',header=header,footer=footer)
